...
public class GanttDAO {
  ...
  public List<Gantt> getGantts(String projectName) {
    String sql = "select ganttNum ,Board_Name from gantts,boards where gantts.Project_Name=? && gantts.Board_Num = boards.Board_Num";
    return jdbc.list(sql, new PreparedStatementSetter() {
      ...
      @Override
      public Gantt mapRow(ResultSet rs) throws SQLException {
        return new Gantt(rs.getInt("ganttNum"), rs.getString("Board_Name"));
      }
    });
  }
