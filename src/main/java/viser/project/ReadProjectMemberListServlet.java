...
public class ReadProjectMemberListServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    List<ProjectMember> memberlist = new ArrayList(); // 형근: chat멤버 목록을 가져오기
                                                      // 위하여 LIST 객체생성
    ProjectDAO projectDao = new ProjectDAO();

    try {
      memberlist = projectDao.getProjectMemberList((String) session.getAttribute("projectName")); // 형근:
                                                                                                  // 멤버목록을
                                                                                                  // LIST
                                                                                                  // 객체에
                                                                                                  // 담는다.
      ...
      for (ProjectMember pm : memberlist) { // 형근: 로그인한 유저가 관리자 권한을 가지고
                                            // 있는지 체크해주는 반복문
